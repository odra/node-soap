...
DefinitionsElement.prototype.addChild = function(child) {
    ...
    if (child instanceof TypesElement) {
        ...
        process.env.DEBUG && console.log('typeof child', typeof child);
        process.env.DEBUG && console.log('child', child);
        ...
        self.processIncludes(function(err) {
            ...
            for(var bindingName in bindings) {
                ...
                for(var methodName in methods) {
                    ...
                    var outputName = null;
                    if (methods[methodName].output == null) {
                        process.env.DEBUG && console.error('no output for methodName, stubbing instead.', methodName, methods[methodName]);
                    } else {
...
WSDL.prototype.describeServices = function() {
  ...
  for (var name in this.services) {
    ...
    services[name] = this._describeWithXmlBodies(service);
  }
  return services;
};

WSDL.prototype._describeWithXmlBodies = function(service){
  var self = this,
  serviceDescription = service.description(this.definitions);
  Object.keys(serviceDescription).forEach(function(portKey){
    var portDescription = serviceDescription[portKey],
    port = service.ports[portKey],
    binding = port.binding,
    methods = binding.methods;
    Object.keys(portDescription).forEach(function(methodKey){
      var methodDescription = portDescription[methodKey],
      method = methods[methodKey],
      input = method.input,
      args = {};

      // Spoof args with ?s
      Object.keys(methodDescription.input).forEach(function(key){
        args[key] = "?";
      });
      var xmlMessageBody = self.objectToDocumentXML(input.$name, args, input.targetNSAlias, input.targetNamespace, input.$type);
      methodDescription.xmlBody = self._getRequestXml('', xmlMessageBody);
    });
...
WSDL.prototype._getRequestXml = function(encoding, message){
    ...
    if (Array.isArray(obj)) {
        ...
        for (var name in obj) {
            ...
            if (name.indexOf(':') > -1) {
                ...
                name = nameParts[2];
                xmlns = nameParts[1];
                xmlnsAttrib = ' xmlns:'+namespace+'="'+xmlns+'"'+' xmlns="'+xmlns+'"';
                process.env.DEBUG && console.log('overwriting xml values', namespace, ns, name, xmlns, xmlnsAttrib);
            }
            parts.push(['<',ns,name,xmlnsAttrib,'>'].join(''));
