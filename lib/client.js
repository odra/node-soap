...
Client.prototype._definePort = function(port, endpoint) {
  ...
  for (var name in methods) {
    ...
    def[name].toXML = this._defineXmlGetter(methods[name], location);
    def[name].asJSON = this._defineMethod(methods[name], location);
    def[name].asXML = this._defineMethod(methods[name], location);
...
Client.prototype._defineXmlGetter = function(method, location) {
  var self = this;
  return function(args) {
    var input = method.input,
    encoding = '',
    message = (typeof args === 'string') ? args : self.wsdl.objectToDocumentXML(input.$name, args, input.targetNSAlias, input.targetNamespace, input.$type);

    return self.getRequestXml(encoding, message);
  };
};
...
Client.prototype._invoke = function(method, args, location, callback, options, extraHeaders) {
  ...
  if (typeof message === 'string' && message.indexOf('soap:Envelope') != -1) {
    xml = message;
  }
